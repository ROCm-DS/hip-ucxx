...
Please note that the number of open files required may differ according to the application, further investigation may be required to find optimal values.
...
UCX respects the operating system's limit of socket ``listen()`` backlog, known in userspace as SOMAXCONN. This limit may cause new endpoints connecting to a listener to hang if too many connections happen to be initiated too quickly.
...
For most Linux distros, the default limit is 128. To increase that limit to 65535 for example, the user may run the following (requires root or sudo permissions):
